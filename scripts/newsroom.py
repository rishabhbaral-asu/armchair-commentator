...
def load_whitelist():
    ...
    with open(WHITELIST_PATH, "r") as f:
        return [t.lower().strip() for t in json.load(f)]

def is_approved_game(event, whitelist):
    """Strictly checks if either team is in your manual whitelist."""
    try:
    ...
    # Check if ANY whitelist string appears in EITHER team name
    for team in teams:
        t_clean = team.lower().strip()
        for target in whitelist:
            if target in t_clean: 
                return True
    return False
...
class Storyteller:
    ...
    def write_body(self):
        if self.g['status'] == 'pre': 
            ...
            return f"""
            <div class='story-container'>
                <h2 class='story-headline'>Final: {self.h['name']} {self.h['score']} - {self.a['score']} {self.a['name']}</h2>
            </div>"""
